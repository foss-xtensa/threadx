    Express Logic's ThreadX for Xtensa Configurable Processors

                            Using the Xtensa Tools

0.  Xtensa Configuration Requirements and Restrictions
...
ThreadX for Xtensa processors is delivered on a single CD-ROM
compatible disk. It comes in source code form with a Makefile that works
for any host platform supported by Xtensa Tools (Windows, Linux), and .bat
files for Windows to conform with other ThreadX ports.  These instructions
are written for Windows users, but can easily be understood and adapted
to other host platforms.

The entire distribution can be found in the sub-directory: threadx

To install ThreadX to your hard-disk, either run the supplied installer 
program Setup.exe (on Windows) or copy the distribution from the CD manually. 

To copy the ThreadX distribution manually, create a threadx directory on your 
hard-disk (we recommend C:\threadx\xtensa\gnu) and copy all the contents 
...
D:\threadx> xcopy /S *.* C:\threadx\xtensa\gnu
